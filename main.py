...
simple_client = clients.SimplePythonClient()
...
class LocalGame(Game):
    ...
    def __init__(self, clients, timeout=999):
    ...
    def show_losers(self):
        for player in self.losers:
            label = '{} ﾐｲﾑ巾ｱﾑ巾ｻ, ﾑﾐｵﾐｷﾑσｻﾑ袴ひｰﾑ� {}'.format(player.name, player.score)
            self.scene.append_label_to_leaderboard(label, player.color)

    def show_score(self):
        for player in self.players:
            label = '{} ﾑﾐｵﾐｷﾑσｻﾑ袴ひｰﾑ� {}'.format(player.name, player.score)
            self.scene.append_label_to_leaderboard(label, player.color)
